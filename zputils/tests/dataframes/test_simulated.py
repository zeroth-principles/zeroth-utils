...
def test_default_distribution():
    rr = SimulatedDataFrame(params = dict(seed = 0, freq = "B", distribution = partial(np.random.normal, loc = 0.0, scale = 1.0)))
...
def test_invalid_distribution_dict():
    with pytest.raises(ValueError, match="distribution should be functools partial function specifying numpy method"):
        rr = SimulatedDataFrame(params=dict(seed = 0, freq = "B", distribution= np.random.normal))
        rr(entities=['A'], period=('2022-01-01', '2022-01-05'))


def test_date_range_generation():
    rr = SimulatedDataFrame(params = dict(seed = 0, freq = "D", distribution = partial(np.random.normal, loc = 0.0, scale = 1.0)))
